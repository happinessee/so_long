	...
	while (i < map.rows)
	{
		...
		while (j < map.columns)
		{
			if (map.map[i][j] == '1')
				...
				mlx_put_image_to_window(map.mlx_info.mlx, map.mlx_info.win, map.img_set.img[1], j * 64, i * 64);
			else if (map.map[i][j] == 'E')
				mlx_put_image_to_window(map.mlx_info.mlx, map.mlx_info.win, map.img_set.img[3], j * 64, i * 64);
			j++;
		}
		i++;
	}
